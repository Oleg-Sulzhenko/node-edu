...
export default function EditProfileForm({ profileData }) {
  ...
  const [formData, setFormData] = useState(isEmpty(profileData) ? initialFormState : profileData);
  ...
  const onSubmit = async (e) => {
    e.preventDefault();
    await dispatch(createProfile({formData, isEdit: false})).then((response) => {
      if(response.error) {
        response.meta.response.data.errors.forEach((err) => {
           setAlert({ msg: err.msg, alertType: "danger" });
        });
      } else {
        setAlert({ msg: "Success", alertType: "success" });
    ...
    <>
      <form className="form" onSubmit={onSubmit}>
        ...
        <div className="my-2">
          <button 
            type="button" 
            onClick={() => {setAddSocialsVisible(!addSocialsVisible)}} 
